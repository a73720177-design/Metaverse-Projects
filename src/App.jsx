...
const validLlmModel = (value) => LLM_MODELS.includes(value) ? value : LLM_MODELS[0]
...
function PersonaCreator({ token, model, onCreated, onDocuments, onDocumentRemoved }) {
  const requests = useRequestScope()
  ...
  return <form className="persona-form" onSubmit={submit}>
    ...
    <div className="button-row"><button type="button" className="text-btn" disabled={busy} onClick={() => setOpen(false)}>닫기</button><button className="primary-btn" disabled={busy || pendingFiles.length > 0 || !field.trim()}>{busy ? <Spinner label={stage} /> : pendingFiles.length > 0 ? '자료 처리 완료 후 생성' : '페르소나 생성'}</button></div>
  </form>
}

function PersonaEditor({ persona, documents, token, model, onSaved, onChanged, onCancel, onDocuments }) {
  const requests = useRequestScope()
  ...
  return <form className="persona-form persona-edit-form" onSubmit={submit}>
    ...
    <div className="button-row"><button type="button" className="text-btn" disabled={busy} onClick={onCancel}>닫기</button><button className="primary-btn" disabled={busy || !field.trim()}>{busy ? <Spinner label="수정·자료 저장 중" /> : '수정 저장'}</button></div>
  </form>
}

function ChatPanel({ result, token, model, documentIds, chat, update }) {
        ...
        <section className="source-block">
          ...
          <div className="persona-grid">{personas.map((p) => {
            ...
            return <article className={`persona-entry ${selected.includes(p.id) ? 'selected' : ''}`} key={p.id}>
              ...
              {editingPersonaId === p.id && <PersonaEditor persona={p} documents={documents} token={token} model={llmModel} onCancel={() => setEditingPersonaId(null)} onDocuments={(docs) => setDocuments((current) => [...current, ...docs.filter((doc) => !current.some((item) => item.document_id === doc.document_id))])} onChanged={(changed) => setPersonas((items) => items.map((item) => item.id === changed.id ? changed : item))} onSaved={(saved) => { setPersonas((items) => items.map((item) => item.id === saved.id ? saved : item)); setEditingPersonaId(null) }} />}
            </article>
          })}</div>
          <PersonaCreator token={token} model={llmModel} onDocuments={(docs) => setDocuments((items) => [...items, ...docs.filter((doc) => !items.some((item) => item.document_id === doc.document_id))])} onDocumentRemoved={(id) => { setDocuments((items) => items.filter((item) => item.document_id !== id)); setProjectIds((ids) => ids.filter((item) => item !== id)) }} onCreated={(p) => setPersonas((x) => [...x, p])} />
        </section>
      </div>
      <section className="generate-bar"><div><strong>예상 질문 준비</strong><div className="generation-options"><label>LLM 모델<select value={llmModel} disabled={generating} onChange={(e) => setLlmModel(validLlmModel(e.target.value))}>{LLM_MODELS.map((model) => <option key={model} value={model}>{model}</option>)}</select></label><label>질문자별 개수<select value={questionCount} disabled={generating} onChange={(e) => setQuestionCount(Number(e.target.value))}>{Array.from({ length: 10 }, (_, i) => <option key={i + 1}>{i + 1}</option>)}</select></label></div><p>{llmModel === 'qwen3.5:9b' ? '고품질 모델 · 더 많은 메모리와 시간이 필요합니다.' : '기본 모델 · 빠른 생성과 낮은 메모리 사용'} · 발표 자료 {projectIds.length}/20개 · 질문자 {selected.length}명</p>{projectIds.length > 20 && <p role="alert">질문 생성에 사용할 발표 자료를 20개 이하로 줄여주세요.</p>}</div><button className="primary-btn" disabled={generating || !projectIds.length || projectIds.length > 20 || !selected.length || projectDocs.some((d) => !textLength(d))} onClick={makeQuestions}>{generating ? <Spinner label="질문 생성 중" /> : '예상 질문 생성'}</button>{generating && <div className="generate-progress"><LoadingProgress label={`${llmModel} · 질문자 ${selected.length}명 예상 질문 생성 중`} expected="4B는 질문자당 약 15초~2분, 9B와 CPU 환경은 더 걸릴 수 있습니다." slowAfterMs={selected.length * 120000} /></div>}</section>
      {error && <p className="global-error">{error}</p>}
      {results.length > 0 && <section id="practice-workspace" tabIndex="-1"><div className="section-title"><span className="eyebrow">PRACTICE</span><h2>페르소나별 답변 연습</h2><p>질문을 고른 뒤 답변하세요. 각 채팅창은 독립적으로 동작합니다.</p></div><div className={`chat-grid panels-${results.length}`}>{results.map((r) => <ChatPanel key={`${activeSessionId}-${sessionRevision}-${r.persona_id}`} result={r} token={token} model={llmModel} documentIds={practiceDocumentIds} chat={chats[r.persona_id]} update={(fn) => setChats((all) => ({ ...all, [r.persona_id]: fn(all[r.persona_id]) }))} />)}</div></section>}
      <WorkspaceLibrary token={token} model={llmModel} documents={documents} personas={personas} onResume={resumeSession} onChanged={refreshResources} />

...
function PersonaCreator({ token, onCreated, onDocuments }) {
  ...
  async function attachFiles(selectedFiles) {
    ...
    try { await requests.run(async (signal) => {
      for (const file of accepted) {
        if (signal.aborted) break
        setStage(`${file.name} 업로드 중`)
        try {
          const doc = await uploadDocument(file, token, signal)
          if (!signal.aborted) { setFiles((old) => [...old.filter((item) => item.document_id !== doc.document_id), doc]); onDocuments([doc]) }
        ...
        <section className="source-block">
          ...
          <div className="persona-grid">{personas.map((p) => {
            ...
            return <article className={`persona-entry ${selected.includes(p.id) ? 'selected' : ''}`} key={p.id}>
              <button type="button" className="persona-choice" disabled={!selected.includes(p.id) && selected.length === 4} onClick={() => togglePersona(p.id)}><span className="avatar-fallback">{p.name.slice(0, 2)}</span><span><strong>{p.name}</strong><small>{p.role} · {p.age ? `${p.age}세 · ` : ''}자료 {linked.length}개</small></span><b>{selected.includes(p.id) ? '✓' : '+'}</b></button>{p.warnings?.map((w) => <p key={w} className="grounding-note">{w}</p>)}

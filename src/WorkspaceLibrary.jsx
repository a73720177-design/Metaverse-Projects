...
import { chatPromptPreview, groupChatsByConversation, reviewableDocuments } from './workspace-utils.mjs'
...
export default function WorkspaceLibrary({ token, model, modelAvailable = true, documents, presentationDocumentIds = [], personas, onResume, onChanged }) {
  ...
  const reviewDocuments = useMemo(
    () => reviewableDocuments(
      documents,
      presentationDocumentIds,
      personas.flatMap((persona) => persona.documentIds || persona.document_ids || []),
    ),
    [documents, presentationDocumentIds, personas],
  )
  ...
  useEffect(() => {
    ...
    const load = tab === 'reviews' ? api.listReviews : api.listChats
  ...
  useEffect(() => {
    if (documentId && !reviewDocuments.some((document) => document.document_id === documentId)) {
      setDocumentId('')
    }
  }, [documentId, reviewDocuments])
  ...
  async function openConversation(group) {
    ...
    throw new Error('이 대화와 연결된 예상 질문 정보를 찾지 못했습니다. 질문자가 삭제됐는지 확인해주세요.')
  }

  const tabs = [['reviews', '자료 리뷰'], ['chats', '대화 이력']]
  const isEmpty = !busy && !items.length
  ...
  return <section className="source-block workspace-library">
    ...
    <nav className="library-tabs" aria-label="저장 기록 메뉴">{tabs.map(([id, label]) => <button type="button" key={id} aria-pressed={tab === id} disabled={busy} onClick={() => { setItems([]); setTab(id); setReview(null); setError('') }}>{label}</button>)}</nav>
    ...
    {tab === 'reviews' && <div className="review-workspace">
      <aside className="review-sidebar">
        <form className="review-create" onSubmit={(e) => { e.preventDefault(); action(async () => setReview(await api.createReview(agent, documentId, token, model)), true) }}>
          ...
          <label>발표 자료<select value={documentId} onChange={(e) => setDocumentId(e.target.value)} required><option value="">{reviewDocuments.length ? '자료 선택' : '리뷰 가능한 발표 자료 없음'}</option>{reviewDocuments.map((d) => <option key={d.document_id} value={d.document_id}>{d.filename}</option>)}</select></label>
          <button className="primary-btn" disabled={busy || !modelAvailable || !agent || !documentId}>리뷰 생성</button>
          {!modelAvailable && <p className="form-error" role="alert">설치된 LLM 모델이 없어 새 리뷰를 생성할 수 없습니다.</p>}
        </form>
    ...
    {tab === 'chats' && <div className="conversation-history">{chatGroups.map((group) => {
      ...
      return <article key={group.conversationId || first.message_id}>
        ...
        <details><summary>저장된 대화 내용 보기</summary>{group.messages.map((chat) => { const text = chatPromptPreview(chat.message); return <div className="history-turn" key={chat.message_id}><p><b>발표자</b>{text.answer}</p><p><b>질문자</b>{chat.answer}</p><Evidence sources={chat.sources} />{chat.grounding && <small>{!chat.grounding.checked ? '근거 검증 미완료' : `근거 점수 ${Math.round(chat.grounding.score * 100)}%`}</small>}<button type="button" className="danger-action" disabled={busy} onClick={() => { if (window.confirm('이 대화를 영구 삭제하시겠습니까? 삭제 후 복구할 수 없습니다.')) action(() => api.deleteChat(chat.message_id, token), true) }}>영구 삭제</button></div>})}</details>
      </article>
    })}</div>}

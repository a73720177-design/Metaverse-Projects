...
export default function WorkspaceLibrary({ token, documents, personas, onResume, onChanged }) {
  ...
  return <section className="source-block workspace-library">
    ...
    {tab === 'reviews' && <>
      ...
      {review && <article><h3>리뷰 결과</h3><CoverageNotice coverage={review.coverage} /><p className="grounding-note">{review.feedback.source_check_performed ? '인용문·파일·구간의 원문 일치를 검사했습니다.' : '이전 결과: 인용 검증 정보가 없습니다. 다시 생성해주세요.'} 외부 사실 검증은 수행하지 않았으며, 인용 일치가 판단의 정확성을 보장하지는 않습니다.</p>{review.feedback.verification_warnings?.map((w) => <p className="grounding-note" key={w}>{w}</p>)}<p><strong>좋은 점</strong> {review.feedback.positive}</p><Evidence sources={review.feedback.positive_sources} /><p><strong>보완할 점</strong> {review.feedback.negative}</p><Evidence sources={review.feedback.negative_sources} />{review.claims.map((c, i) => <div key={i}><p>{c.claim} · {{ supported: '근거 있음', partially_supported: '일부 근거 있음', contradicted: '자료와 상충', overgeneralized: '과도한 일반화', insufficient_evidence: '근거 부족', not_verifiable: '검증 불가' }[c.verdict] || c.verdict}</p><Evidence sources={c.sources} /></div>)}<ol>{review.questions.map((q, i) => <li key={i}>{q}</li>)}</ol></article>}
    </>}

...
def check_ollama_health() -> bool:
    ...
    try:
        if _provider() == "vllm":
            ...
            if _vllm_model(None) not in served:
                return False
        ...
        required = {OLLAMA_EMBEDDING_MODEL}
        if _provider() == "ollama":
            required.update({OLLAMA_CHAT_MODEL, OLLAMA_REVIEW_MODEL,
                             OLLAMA_QUESTION_MODEL, OLLAMA_SUMMARY_MODEL})
        def canonical(name):
            return name if ":" in name.rsplit("/", 1)[-1] else name + ":latest"
        return {canonical(name) for name in required} <= {canonical(name) for name in available}
    except (requests.RequestException, LLMError, ValueError, AttributeError, TypeError):
        return False
...
def get_runtime_diagnostics() -> dict:
    ...
    ollama_generation_models = {
        ...
        "question": OLLAMA_QUESTION_MODEL,
        "review": OLLAMA_REVIEW_MODEL,
        "summary": OLLAMA_SUMMARY_MODEL,
    }
    ollama_generation_operational = ollama_reachable and all(
        model_available(name) for name in ollama_generation_models.values()
    )
    generation_operational = (
        vllm_reachable and vllm_model_available
        if provider == "vllm"
        else ollama_generation_operational
    )
    ...
    vllm_gpu_configured = os.getenv("VLLM_GPU_ENABLED", "false").strip().lower() in {
        "1", "true", "yes", "on",
    }
    vllm_gpu_operational = (
        provider == "vllm" and vllm_gpu_configured
        and vllm_reachable and vllm_model_available
    )
    if vllm_gpu_operational:
        accelerator_mode = "gpu"
        accelerator_operational = True
        gpu_percent = 100.0
    else:
        accelerator_mode = (
        ...
        accelerator_operational = None if accelerator_mode == "idle" else total_vram_bytes > 0
    selectable_models = [
    ...
    return {
        ...
        "features": {
            ...
            "ollama_generation": {
                "enabled": True,
                "operational": ollama_generation_operational,
                "mode": "active" if provider == "ollama" else "standby",
                "models": ollama_generation_models,
            },
            "ollama_embedding": {
                "enabled": True,
            ...
            "gpu_acceleration": {
                "enabled": True,
                "operational": accelerator_operational,

...
from app.models.persona import (
    ...
    PersonaTrait, Evidence, active_trait,
)
...
def quote_key(value: str) -> str:
    # Ignore typography and layout whitespace only. Keep punctuation, negation
    # and word order: paraphrases and stitched quotations must not pass.
    return "".join(unicodedata.normalize("NFKC", value).split())


def verified_quote(summary: str, source: str, value: str) -> bool:
    key = quote_key(summary)
    significant = sum(c.isalnum() for c in key)
    # A single character/common fragment cannot substantiate a long trait.
    if significant < 4 and not (significant >= 2 and key == quote_key(value)):
        return False
    return key in quote_key(source)
...
class PersonaService:
    ...
    def _verify_traits(persona: PersonaProfile, request: PersonaGenerationRequest) -> None:
        ...
        for trait in [*persona.expertise, *persona.evaluation_style]:
            evidence = []
            seen = set()
            for e in trait.evidence:
                key = (e.source_id, quote_key(e.summary))
                if (e.confidence > 0 and key not in seen and e.source_id in sources
                        and verified_quote(e.summary, sources[e.source_id], trait.value)):
                    evidence.append(e)
                    seen.add(key)
            trait.evidence = evidence
            if trait.status == EvidenceStatus.CONFLICTING:
                trait.confidence = 0
                trait.exclusion_reason = "conflicting"
            elif not trait.evidence:
                trait.status = EvidenceStatus.UNKNOWN
                trait.confidence = 0
                trait.exclusion_reason = "missing_quote"
            elif trait.status == EvidenceStatus.UNKNOWN:
                trait.exclusion_reason = "model_unknown"
            elif trait.confidence <= 0:
                trait.status = EvidenceStatus.UNKNOWN
                trait.exclusion_reason = "zero_confidence"
                ...
                trait.exclusion_reason = None
            else:
                trait.exclusion_reason = None
        if not any(active_trait(t) for t in [*persona.expertise, *persona.evaluation_style]):
            # User-provided simulation instructions are valid declared criteria,
            # not proof of a real person's credentials. Never fabricate expertise.
            declared = request.description.strip()[:300]
            if declared:
                persona.evaluation_style = persona.evaluation_style[:9]
                persona.evaluation_style.append(PersonaTrait(
                    value=declared, status=EvidenceStatus.USER_STATED, confidence=1.0,
                    evidence=[Evidence(source_id="description", summary=declared[:60], confidence=1.0)],
                ))
    ...
    async def update_documents(
        ...
        if current is None:
            raise PersonaNotFoundError("질문자를 찾을 수 없습니다.")
        # Recompute criteria from the newly attached references as one update.
        return await self.update(agent_id, PersonaUpdateRequest(
            name=current.name, description=current.description, document_ids=document_ids,
        ), owner_id)

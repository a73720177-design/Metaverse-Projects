...
class PracticeService:
    ...
    async def generate_expected_questions(
        ...
        async def generate_for_persona(persona) -> PersonaQuestionResult:
            ...
            # Bounded batches let the model replace rejected candidates without
            # consuming the context budget with an ever-larger output request.
            for attempt in range(6 if count else 0):
                remaining = count - len(questions)
                # Small batches leave room for source text in a 4K context.
                # Increasing the requested output on retry previously caused 422.
                candidate_count = min(3, remaining)

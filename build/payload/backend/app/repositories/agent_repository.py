...
from app.models.persona import PersonaHistoryItem, PersonaProfile, invalidate_reference_traits
...
class InMemoryAgentRepository:
    ...
    async def unlink_document(self, document_id: UUID, owner_id: UUID) -> None:
        for agent_id, (stored_owner, persona) in list(self._agents.items()):
            ...
            self._agents[agent_id] = (
                stored_owner,
                invalidate_reference_traits(persona).model_copy(update={
                    "document_ids": [
    ...
    async def set_documents(
        ...
        if stored is None or stored[0] != owner_id or stored[1].deleted_at is not None:
            return None
        current = stored[1]
        if set(current.document_ids) != set(document_ids):
            current = invalidate_reference_traits(current)
        persona = current.model_copy(
...
class PostgresAgentRepository:
    ...
    async def unlink_document(self, document_id: UUID, owner_id: UUID) -> None:
        async with get_session_factory()() as session:
            affected = (await session.scalars(
                select(AgentTable).join(AgentDocumentTable).where(
                    AgentTable.owner_id == owner_id,
                    AgentDocumentTable.document_id == document_id,
                ).with_for_update(of=AgentTable)
            )).all()
            for row in affected:
                updated = invalidate_reference_traits(_to_history(row)).model_dump(mode="json")
                row.expertise = updated["expertise"]
                row.evaluation_style = updated["evaluation_style"]
    ...
    async def set_documents(
        ...
        async with get_session_factory()() as session:
            ...
            if row is None:
                return None
            previous_ids = await self._document_ids(session, agent_id)
            if set(previous_ids) != set(document_ids):
                updated = invalidate_reference_traits(_to_history(row)).model_dump(mode="json")
                row.expertise = updated["expertise"]
                row.evaluation_style = updated["evaluation_style"]
            await session.execute(

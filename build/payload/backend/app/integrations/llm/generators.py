...
class HttpQuestionGenerator:
    ...
    async def generate(self, persona, document, instructions, *, question_count=5,
        ...
        try:
            ...
            raise ReviewGeneratorError("질문 생성 서버에 연결하지 못했습니다. 서버 상태를 확인해 주세요.") from exc
        except LlmServiceResponseError as exc:
            if "HTTP 422" in str(exc):
                message = "질문 생성 입력이 모델의 길이 한도 또는 요청 형식에 맞지 않습니다."
            else:
                message = "질문 생성 서버가 유효한 응답을 반환하지 못했습니다. 다시 생성해 주세요."
            raise ReviewGeneratorError(message) from exc
